             ...
             Main(Section(A("← All open roles", href=careers_path, cls="c-link back"),
                  ...
                  Div(Section(P(job.get("summary") or "", cls="c-lede"),
                      ...
                      Section(H2("Apply now"),
                              ...
                              Form(Div(Label("First name"), Input(name="first_name", value=values.get("first_name", ""), required=True, aria_label="First name"), cls="field"),
                                   Div(Label("Last name"), Input(name="last_name", value=values.get("last_name", ""), required=True, aria_label="Last name"), cls="field"),
                                   Div(Label("Email"), Input(type="email", name="email", value=values.get("email", ""), required=True, aria_label="Email"), cls="field"),
                                   Div(Label("Phone"), Input(name="phone", value=values.get("phone", ""), aria_label="Phone"), cls="field"),
                                   Div(Label("Location"), Input(name="location", value=values.get("location", ""), aria_label="Location"), cls="field"),
                                   Div(Label("CV · PDF, DOCX, TXT or MD"), Input(type="file", name="cv", accept=".pdf,.docx,.txt,.md", required=True, aria_label="CV"), cls="field"),
                                   *custom_fields,
                                   Div(Label("Cover note"), Textarea(values.get("cover_note", ""), name="cover_note", aria_label="Cover note"), cls="field"),
...
def editor(job_id: int | None = None, *, saved: str = "", error: str = ""):
    ...
    def field(label, name, *, kind="text", value=None, required=False):
        ...
        control = Textarea(value or "", name=name, cls="prompt-box", style="min-height:110px", aria_label=label) if kind == "textarea" else Input(type=kind, name=name, value=value or "", required=required, cls="hr-inp", aria_label=label)
    ...
    return (
        ...
        Form(Div(H3("Role details"), cls="card-header"),
             ...
             Div(Select(*dept_options, name="dept_id", cls="hr-inp", aria_label="Department"),
                 Select(*manager_options, name="hiring_manager_id", cls="hr-inp", aria_label="Hiring manager"),
                 Input(type="number", name="headcount", min="1", value=data.get("headcount") or 1, cls="hr-inp", aria_label="Headcount"),
                 cls="inline-form", style="margin:12px 0;flex-wrap:wrap"),
             Div(field("Location", "location", value=data.get("location")),
                 Div(Label("Remote policy"), Select(*[Option(x, value=x, selected=(data.get("remote_policy") == x)) for x in ("Onsite", "Hybrid", "Remote")], name="remote_policy", cls="hr-inp", aria_label="Remote policy")),
                 Div(Label("Employment type"), Select(*[Option(x, value=x, selected=(data.get("employment_type") == x)) for x in ("Permanent", "Contract", "Intern", "Temporary")], name="employment_type", cls="hr-inp", aria_label="Employment type")),
                 cls="grid-2"),
...
def site_settings(*, saved: str = ""):
    ...
    return (
        ...
        Form(Div(H3("Brand"), cls="card-header"),
             Div(Label("Site name"), Input(name="name", value=site["name"], required=True, cls="hr-inp", aria_label="Site name")),
             Div(Label("Headline"), Input(name="headline", value=site.get("headline") or "", cls="hr-inp", aria_label="Headline")),
             Div(Label("Introduction"), Textarea(site.get("introduction") or "", name="introduction", cls="prompt-box", aria_label="Introduction")),
             Div(Label("Brand colour"), Input(type="color", name="brand_color", value=site["brand_color"], aria_label="Brand colour"),
                 Label("Accent colour"), Input(type="color", name="accent_color", value=site["accent_color"], aria_label="Accent colour"), cls="inline-form"),
             Div(Label("Logo URL"), Input(type="url", name="logo_url", value=site.get("logo_url") or "", cls="hr-inp", aria_label="Logo URL")),
             Div(Label("Privacy policy URL"), Input(name="privacy_policy_url", value=site.get("privacy_policy_url") or "", cls="hr-inp", aria_label="Privacy policy URL")),
