        ...
        Form(
            Div(Div("Drop a CV here, or choose a file", cls="big"),
                ...
                Input(type="file", name="cv", accept=".pdf,.docx,.txt,.md", required=True,
                      style="margin-top:12px;", aria_label="CV file"),
                cls="drop-zone"),
            Div(Select(Option("No requisition. Add to talent pool", value="0"),
                       *[Option(f"{j['title']} ({j['code']})", value=str(j["id"]),
                       ...
                       name="job_id", cls="hr-inp", style="flex:1;min-width:220px;", aria_label="Requisition"),
                Select(*[Option(s, value=s) for s in talent.SOURCES], name="source", cls="hr-inp", aria_label="Source"),
...
def _offer_panel(app, cand):
    ...
    return Div(Div(H3("Make an offer"), cls="card-header"),
               ...
               Form(Input(name="salary", type="number", step="any", placeholder="Salary",
                    ...
                    Input(type="date", name="start_date", cls="hr-inp", required=True, aria_label="Alguskuupäev"),
                    Input(type="date", name="expires_on", cls="hr-inp", aria_label="Kehtiv kuni"),
