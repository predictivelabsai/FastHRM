#!/usr/bin/env bash
# Build the FastHRM user guide as a landscape slide deck — PDF + PPTX.
#
#   bash scripts/build_user_guide.sh                       # newest dated guide
#   bash scripts/build_user_guide.sh docs/fasthrm_user_guide_2026-07-20.md
#
# Pipeline:
...
# Screenshots come from screenshots/ — refresh them with:
#   DEMO_BASE_URL=http://localhost:5010 python scripts/capture_guide_screenshots.py
#
...
if [ "${1:-}" != "" ]; then
  ...
  SRC="$(ls -1 fasthrm_user_guide_*.md 2>/dev/null | sort | tail -1)"
fi
[ -n "${SRC:-}" ] && [ -f "$SRC" ] || { echo "⚠ no guide markdown found (docs/fasthrm_user_guide_*.md)"; exit 1; }

BASE="${SRC%.md}"
HTML="${BASE}.html"
PDF="${BASE}.pdf"
PPTX="${BASE}.pptx"
TITLE="FastHRM — User Guide"
...
sed -i -E "s|content: \"[^\"]*hrm\.fastsme\.com\"|content: \"v${VERSION} · ${GEN_DATE} · hrm.fastsme.com\"|" assets/guide.css
...
echo "✓ FastHRM user guide built (v${VERSION}, ${GEN_DATE}): PDF + PPTX."
